...
int fixedArmMotorPin = 5;
int rotatingArmMotorPin = 3;
int electromagPin = 6;
...
pinMode(fixedArmMotorPin, OUTPUT);
pinMode(rotatingArmMotorPin, OUTPUT);
...
void loop()
{
  // interpret incoming signals from Unity
  if (Serial.available() > 0)  {
    String incoming = Serial.readString();
    	String cmd1 = GetValue(incoming, ':', 0);
      String cmd2 = GetValue(incoming, ':', 1);
      int pin;
      if (cmd1 == 'L') {
        pin = rotatingArmMotorPin;
      } else if (cmd1 == 'R') {
        pin = fixedArmMotorPin;
      } else if (cmd1 == 'M') {
        pin = electromagPin;
      }
      if (cmd2 == '0') {
